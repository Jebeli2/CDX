namespace CDX.Actors
{
    ...
    public class Actor : Resource
    {
        ...
        private readonly List<ISortableSprite> sprites = new();
        ...
        public Actor(string name) : base(name, ContentFlags.Data)
        {
            direction = 7;
        ...
        public void Update(FrameTime time)
        {
            ...
            {
                Dead = true;
                //map.Collision.Unblock(PosX, PosY);
                //Logger.Debug($"{this} dead and unblocked @({PosX}/{PosY})");
                //manager.EntityKilled(this);
            }
        ...
        public IEnumerable<ISortableSprite> GetSprites()
        {
            UpdateSprites();
            List<ISortableSprite> list = new List<ISortableSprite>(sprites);
            return list;
        }
        ...
        private List<ISortableSprite> GetCurrentSprites()
        {
            List<ISortableSprite> list = new List<ISortableSprite>();
            long prio = 1;
            if (visual != null)
            {
                foreach (ISprite sprite in visual.CurrentSprites)
                {
                    MapPosSortableSprite s = new MapPosSortableSprite(sprite)
                    {
                        MapPosX = posX,
                        MapPosY = posY,
                        BasePrio = prio
