namespace CDX.Actors
{
    ...
    public class MultiPartVisual : IVisual
    {
        ...
        public IEnumerable<ISprite> CurrentSprites => activeAnimations.Select(x => x.CurrentSprite);

        //public bool IsMoving => string.Equals("run", Animation);
        ...
        {
            bool changed = false;
            foreach (var a in activeAnimations) { if (a.Update(time)) { changed = true; }; }
            return changed;
        }
        ...
        private void ChangeAnimation(int dir, string name)
        {
            ...
            if (count == 0)
            {
                Logger.Error($"Found no animations {name}.");
            }
        }
    }
}
