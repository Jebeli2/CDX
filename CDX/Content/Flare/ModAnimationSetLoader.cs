namespace CDX.Content.Flare
{
    ...
    {
        public ModAnimationSetLoader() : base("Flare Mod AnimationSet Loader")
        {
        }
        ...
        public AnimationSet? Load(string name, byte[]? data)
        {
            ...
            {
                result = LoadAnimationSet(infile, name);
                if (result != null)
                {
                    Logger.Info($"AnimationSet loaded from resource '{name}'");
                }
            }
            return result;
        }
        ...
        private AnimationSet? LoadAnimationSet(FileParser infile, string name)
        {
            ...
            while (infile.Next())
            {
                ...
                {
                    image = ContentManager?.Load<IImage>(infile.GetStrVal());
                }
                ...
                else if (!infile.MatchSection(""))
                {
                    ...
                    else if (infile.MatchKey("frame"))
                    {
                        ...
                        if (image != null)
                        {
                            ...
                            {
                                Logger.Error($"Frame Index Mismatch {index} in file '{name}'");
                            }
                        }
                        else
                        {
                            Logger.Error($"No Image for Animation in file '{name}'");
                        }
                    }
                }
            }
