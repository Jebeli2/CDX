namespace CDX.Content.Flare
{
    ...
    public class ModMapLoader : ModLoader, IResourceLoader<Map>
    {
        ...
        public Map? Load(string name, byte[]? data)
        {
            ...
            {
                result = LoadMap(infile, name);
                if (result != null)
                {
                    result.InitCollision();
                    Logger.Info($"Map loaded from resource '{name}'");
                }
            }
            return result;
        }
        ...
        private Map LoadMap(FileParser infile, string name)
        {
            ...
            int heroPosX = -1;
            int heroPosY = -1;
            MapOrientation orientation = MapOrientation.Isometric;
                    ...
                    case "header":
                        switch (infile.Key)
                        {
                            ...
                            case "hero_pos":
                                ...
                                break;
                            case "orientation": orientation = infile.GetEnumValue(MapOrientation.Isometric); break;
