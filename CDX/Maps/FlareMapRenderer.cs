namespace CDX.Maps
{
    ...
    public class FlareMapRenderer : IMapRenderer
    {
        ...
        public void Update(FrameTime time, Map map)
        {
            map.Parallax?.Update(time);
            map.TileSet?.Update(time);
        }
        public void Render(IGraphics graphics, FrameTime time, Map map, IList<ISortableSprite> front, IList<ISortableSprite> back)
        {
            if (InitMapValues(map))
            {
                if (InitGfxValues(graphics))
                {
                    ...
                    else if (orientation == MapOrientation.Orthogonal)
                    {
        ...
        private void UpdateShakyCam(FrameTime? frameTime = null)
        {
            if (shakyCamDuration > 0)
            {
                if (frameTime != null)
                {
                    shakyCamDuration -= frameTime.ElapsedTime.TotalMilliseconds;
                }
                else
                {
                    shakyCamDuration -= 1.0;
                }
            }
            if (shakyCamDuration > 0)
            {
        ...
        private bool InitGfxValues(IGraphics graphics)
        {
        ...
        private void RenderIso(IGraphics graphics, Map map, IList<ISortableSprite> front, IList<ISortableSprite> back)
        {
        ...
        private void RenderOrtho(IGraphics graphics, Map map, IList<ISortableSprite> front, IList<ISortableSprite> back)
        {

        }

        private void RenderIsoBackObjects(IGraphics graphics, IList<ISortableSprite> r)
        {
            foreach (var s in r) { DrawSprite(graphics, s); }
        }

        private void RenderIsoFrontObjects(IGraphics graphics, MapLayer layer, IList<ISortableSprite> r)
        {
            if (tset == null) return;
            int w = layer.Width;
            int h = layer.Height;
        ...
        private void RenderParallax(IGraphics graphics, MapParallax? parallax, string layerName)
        {
            if (parallax != null)
            {
        ...
        private void RenderParallaxLayer(IGraphics graphics, ParallaxLayer layer)
        {
            if (layer.Image != null)
            {
        ...
        private void RenderIsoCollision(IGraphics graphics, IMapCollision? collision)
        {
            if (tset == null) return;
            if (cset == null) return;
        ...
        private void RenderIsoLayer(IGraphics graphics, MapLayer layer)
        {
            if (tset == null) return;
            int w = layer.Width;
            int h = layer.Height;
        ...
        private void DrawCollision(IGraphics graphics, IMapCollision collision, int x, int y, int sX, int sY)
        {
        ...
        private void DrawTile(IGraphics graphics, MapLayer layer, int x, int y, int sX, int sY)
        {
        ...
        private void DrawSprite(IGraphics graphics, ISortableSprite r)
        {
            if (r.Image != null)
            {
