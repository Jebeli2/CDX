namespace CDX.Screens
{
    ...
    public class Tooltip
    {
        ...
        private void Render(IGraphics gfx, int x, int y)
        {
            if (gfx == null) return;
        ...
        private void CheckBufferSize(IGraphics gfx, int width, int height)
        {
            if (buffer == null || buffer.Width < width || buffer.Height < height)
            {
                buffer?.Dispose();
                buffer = gfx.CreateImage("tooltip", width, height);
            }
        }
        private void CreateBuffer(IGraphics gfx, TooltipData data)
        {
            Size textSize = new();
            ...
            if (buffer != null)
            {
                ...
                if (background != null)
                {
                    ...
                    gfx.DrawImage(background, srcX, srcY, width, height, dstX, dstY, width, height);
                    // right
                ...
                foreach (string line in data.Lines)
                {
                    ...
                    gfx.DrawText(data.Font, line, cursorX, cursorY, Color.White);
                    cursorY += size.Height;
                    cursorY += lineSpacing;
                    idx++;
                }
